...
import {HomeBannerDynamic} from '~/components/HomeBannerDynamic';
...
export async function loader(args: LoaderFunctionArgs) {
  ...
  // Fetch banner home metafields
  let bannerSlots: any[] = [];
  
  try {
    const result = await storefront.query(SHOP_BANNER_HOME_QUERY);
    
    const bannerShop = result?.shop;
    
    if (!bannerShop) {
      console.warn('⚠️ Shop não encontrado na query de banners');
      bannerSlots = [];
    } else {
      // Verificar se o metafield existe
      if (!bannerShop.banners) {
        console.warn('⚠️ Metafield "custom.banners_home" não encontrado no Shopify');
        console.log('📋 Para configurar:');
        console.log('   1. Vá em Settings > Custom data > Metafields');
        console.log('   2. Crie um metafield no Shop com:');
        console.log('      - Namespace: custom');
        console.log('      - Key: banners_home');
        console.log('      - Type: List of metaobjects');
        console.log('   3. Crie metaobjects e vincule-os ao metafield');
        bannerSlots = [];
      } else if (!bannerShop.banners.references || !bannerShop.banners.references.nodes) {
        console.warn('⚠️ Metafield existe mas não tem referências');
        console.log('   Adicione metaobjects ao metafield "custom.banners_home" no Shopify');
        bannerSlots = [];
      } else {
        // Processar lista de banners
        const bannerNodes = bannerShop.banners.references.nodes || [];
        
        if (bannerNodes.length === 0) {
          console.warn('⚠️ Nenhum metaobject encontrado no metafield "banners"');
          console.log('   Adicione metaobjects ao metafield no Shopify Admin');
        } else {
          console.log(`✅ ${bannerNodes.length} banner(s) encontrado(s) no Shopify`);
          
          bannerSlots = bannerNodes.map((banner: any, index: number) => {
            // Processar imagem - pode vir como referência ou URL direta
            const imagemField = banner?.imagem;
            let imageUrl = null;
            let imageAlt = null;
            
            if (imagemField?.reference?.image) {
              // Imagem como referência (MediaImage)
              imageUrl = imagemField.reference.image.url;
              imageAlt = imagemField.reference.image.altText;
            } else if (imagemField?.value) {
              // Imagem como URL direta (string)
              imageUrl = imagemField.value;
            }
            
            const ordem = parseInt(banner?.ordem?.value || '0', 10);
            
            const bannerData = {
              enabled: true, // Assumindo que todos os banners retornados estão habilitados
              imageDesktop: imageUrl ? { url: imageUrl, altText: imageAlt } : null,
              imageMobile: imageUrl ? { url: imageUrl, altText: imageAlt } : null, // Usando a mesma imagem por enquanto
              subtitle: null,
              title: banner?.titulo?.value || null,
              description: null,
              ctaText: null,
              link: banner?.link?.value || null,
              overlay: false,
              limitedTime: false,
              ordem: ordem, // Para ordenação
            };
          ...
          bannerSlots.sort((a, b) => (a.ordem || 0) - (b.ordem || 0));
          
          console.log(`✅ ${bannerSlots.length} banner(s) processado(s) e ordenado(s)`);
        }
      }
    }
  } catch (error) {
    // Se houver erro (metafield não existe ainda), retorna array vazio
    console.error('❌ Erro ao buscar banners:', error);
    if (error instanceof Error) {
      console.error('   Mensagem:', error.message);
      if (error.message.includes('metafield') || error.message.includes('not found')) {
        console.log('📋 O metafield provavelmente não existe. Configure no Shopify:');
        console.log('   Settings > Custom data > Metafields');
        console.log('   Namespace: custom, Key: banners_home');
      }
    }
    bannerSlots = [];
  }
...
export default function Homepage() {
  const {testimonials, bannerSlots} = useLoaderData<typeof loader>();
  const data = useLoaderData<typeof loader>();

  const teaserActive = isTeaserActive();

  // Debug: verificar dados do banner
  useEffect(() => {
    console.log('BannerSlots no componente:', bannerSlots);
    console.log('BannerSlots length:', bannerSlots?.length);
    if (bannerSlots && bannerSlots.length > 0) {
      console.log('Banners habilitados:', bannerSlots.filter((b: any) => b.enabled));
    }
  }, [bannerSlots]);
  ...
  return (
    <>
      ...
      <div className="home gap-10 flex flex-col">
        {/* Banner dinâmico da homepage */}
        {bannerSlots && bannerSlots.length > 0 ? (
          <HomeBannerDynamic slots={bannerSlots} />
        ) : (
          <div className="w-full px-4">
            <div className="w-full h-[400px] md:h-[500px] bg-[#f5f5f5] rounded-2xl flex items-center justify-center border-2 border-dashed border-[#e0e0e0]">
              <div className="text-center px-4 max-w-2xl">
                <p className="text-text-sub-400 text-label-md mb-2">
                  Área do Banner
                </p>
                <p className="text-text-sub-300 text-paragraph-sm mb-4">
                  Configure os banners no Shopify para exibi-los aqui
                </p>
                <div className="text-left bg-white/50 rounded-lg p-4 text-paragraph-xs text-text-sub-500">
                  <p className="font-semibold mb-2">📋 Como configurar:</p>
                  <ol className="list-decimal list-inside space-y-1">
                    <li>Vá em <strong>Settings &gt; Custom data &gt; Metafields</strong></li>
                    <li>Crie um metafield no <strong>Shop</strong> com:
                      <ul className="list-disc list-inside ml-4 mt-1">
                        <li>Namespace: <code className="bg-gray-100 px-1 rounded">custom</code></li>
                        <li>Key: <code className="bg-gray-100 px-1 rounded">banners_home</code></li>
                        <li>Type: <strong>List of metaobjects</strong></li>
                      </ul>
                    </li>
                    <li>Crie metaobjects com os campos: <code className="bg-gray-100 px-1 rounded">imagem</code>, <code className="bg-gray-100 px-1 rounded">titulo</code>, <code className="bg-gray-100 px-1 rounded">link</code>, <code className="bg-gray-100 px-1 rounded">ordem</code></li>
                    <li>Vincule os metaobjects ao metafield</li>
                  </ol>
                </div>
              </div>
            </div>
...
const SHOP_BANNER_HOME_QUERY = `#graphql
  query ShopBannerHome {
    shop {
      banners: metafield(namespace: "custom", key: "banners_home") {
        references(first: 10) {
          nodes {
            ... on Metaobject {
              id
              imagem: field(key: "imagem") {
                value
                reference {
                  ... on MediaImage {
                    image {
